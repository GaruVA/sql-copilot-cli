...
namespace NL2SQL_CLI
{
    ...
    public class SqlQueryService
    {
        ...
        private string BuildConversationalPrompt(string naturalLanguageQuery)
        {
            ...
            sb.AppendLine("1. Write exactly ONE SQL query");
            sb.AppendLine("2. Start immediately with SELECT (no explanation before query)");
            sb.AppendLine("3. Use ONLY these tables: " + string.Join(", ", _validTableNames.OrderBy(t => t)));
            sb.AppendLine("4. Use ONLY columns shown in schema above");
            sb.AppendLine("5. End query with semicolon (;) then ### marker");
            sb.AppendLine();
            sb.AppendLine("T-SQL Rules (Microsoft SQL Server):");
            sb.AppendLine("- Always use dbo. prefix for table names");
            sb.AppendLine("- Date arithmetic: DATEADD(MONTH, -1, GETDATE())");
            sb.AppendLine("- Row limits: TOP N (not LIMIT N)");
            sb.AppendLine("- No NULLS LAST, ILIKE, or interval syntax");
            sb.AppendLine();
            sb.AppendLine("### Examples:");
            sb.AppendLine();
            sb.AppendLine("Q: How many orders do we have?");
            sb.AppendLine("A: SELECT COUNT(*) AS TotalOrders FROM dbo.Orders;");
            sb.AppendLine();
            sb.AppendLine("Q: Show products with categories");
            sb.AppendLine("A: SELECT p.ProductName, c.CategoryName FROM dbo.Products p JOIN dbo.Categories c ON p.CategoryID = c.CategoryID;");
            sb.AppendLine();
            sb.AppendLine("Q: Orders from last 30 days");
            sb.AppendLine("A: SELECT * FROM dbo.Orders WHERE OrderDate >= DATEADD(DAY, -30, GETDATE());");
            sb.AppendLine();
